...
int locInSubSymTab(string iden){

// 符号表越上面的离现在越近，越下面说明声明的时间越早
	int i = symtable.indexTab[symtable.subTotal];//当前运行的分程序开头
...
int loc(string iden, bool isFunc){
	if(isFunc){
...
// 符号表爆栈
	...
	if(obj == objFUNCTION){//先检查函数，因为函数可以利用分函数索引更快查找
		...
		// 检查局部变量有无重复定义
		for(int i = symtable.indexTab[symtable.subTotal];
			i < symtable.level; i++){
...
void headProc(){
	...
	alive = 1;//全局变量定义部分，打开入栈开关
	constProc();
	varProc();

	// 结束全局变量定义，进入函数定义部分
	isGlobalDec = 0;
	alive = 0;//函数声明部分，不需要入栈
	funcProc();
}
